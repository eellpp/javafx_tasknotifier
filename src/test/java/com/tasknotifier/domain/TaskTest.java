...
class TaskTest {
    ...
    void rejectsBlankTitle() {
        assertThrows(IllegalArgumentException.class, () -> new Task(null, "  ", "", LocalDateTime.now(), Priority.MEDIUM,
                TaskStatus.TODO, List.of(), "task-detail/a.md", RecurrenceRule.none(), ReminderSettings.defaults(), LocalDateTime.now()));
    }

    @Test
    void createsValidTask() {
        Task task = new Task(null, "Task", "summary", LocalDateTime.now(), Priority.HIGH,
                TaskStatus.TODO, List.of("work"), "task-detail/a.md", RecurrenceRule.none(), ReminderSettings.defaults(), LocalDateTime.now());
